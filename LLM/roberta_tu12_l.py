...
class CustomModel(torch.nn.Module):
    def __init__(self, num_classes, input_dim=2): # todo
        super().__init__()
        self.roberta = RobertaModel.from_pretrained(LOCAL_MODEL_PATH)
        ...
        self.classifier = torch.nn.Sequential(
            torch.nn.Linear(self.roberta.config.hidden_size, 256),
            torch.nn.ReLU(),
    ...
    def forward(self, input_ids=None, **kwargs):
        # 扩展维度并投影到RoBERTa嵌入空间
        # x = input_ids.unsqueeze(-1) # todo 若数据是2维，需要改一下.
        x = input_ids.view(input_ids.size(0), 512, 2) # 若数据是2维
        embedded = self.input_proj(x)

        # outputs = self.roberta(inputs_embeds=embedded)
        ...
        # 应用BERT的embedding层
        hidden_states = self.roberta.embeddings(
            input_ids=None,
            position_ids=None,
            token_type_ids=None,
...
def main(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='roberta-best-model-{epoch:02d}-{val_f1:.4f}',
        save_top_k=1,
        mode='max'
    )
