...
# 1. 定义本地模型路径（BERT）
# ---------------------
LOCAL_MODEL_PATH = "./deberta_v3_base"  # 修改为BERT本地路径
...
class CustomModel(nn.Module): # todo 可训练参数 639746
    def __init__(self, num_classes, input_dim=2):
        super().__init__()
        self.deberta = DebertaV2Model.from_pretrained(LOCAL_MODEL_PATH)  #todo 实际使用时替换为LOCAL_MODEL_PATH
    ...
    def add_adapters(self):
        """在DeBERTa的每个encoder层后添加Adapter"""
        for i in range(len(self.deberta.encoder.layer)):
            # 为注意力输出添加Adapter
    ...
    def forward(self, input_ids=None, **kwargs):
        ...
        if attention_mask is not None:
            extended_attention_mask = self.deberta.get_extended_attention_mask(
                attention_mask,
                input_ids.shape,
                input_ids.device
            )
        ...
        # 应用最终层归一化
        hidden_states = self.deberta.encoder.final_layer_norm(hidden_states)
    ...
    def setup_training_params(model):
        ...
        for name, param in model.named_parameters():
            if 'adapter' in name or 'classifier' in name or 'input_proj' in name:
...
class LoraBertLightningModule(LightningModule):
    def __init__(self, num_classes=2, input_dim=2, lr=1e-4): # TODO
        ...
        self.peft_config = LoraConfig(
            ...
            lora_alpha=32,
            lora_dropout=0.1,
            # target_modules=["query", "key", "value"], # TODO ,DEBERT的模块可能有些不同
            target_modules=["query_proj", "key_proj", "value_proj"],
            # idea2.
            # target_modules=["query_proj", "key_proj", "value_proj", "LayerNorm"],
            # idea3.
            # target_modules=["query_proj", "key_proj", "value_proj", "encoder.layer.*.attention.output.LayerNorm","encoder.layer.*.output.LayerNorm"],
            # idea4.就是加上那个adapter了.
            bias="none",
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default='./deberta_saved12_k_L', help='Output directory for saved model') # TODO
